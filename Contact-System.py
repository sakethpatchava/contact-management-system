...
def Submit():
    if f_name.get() == "" or m_name.get() == "" or l_name.get() == "" or gender.get() == "" or age.get() == "" or home_address.get() == "" or phone_number.get() == "" or Email.get() == "" or nationality.get() == "":
        ...
        cursor.execute(
            ...
            int(phone_number.get()), str(Email.get()), str(nationality.get())))
        conn.commit()
        cursor.execute("SELECT * FROM `contactable` ORDER BY `last_name` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def Update():
    if gender.get() == "":
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE `contactable` SET `first_name` = ?, `middle_name` = ? , `last_name` = ?, `gender` =?, `age` = ?,  "
            "`home_address` = ?, `phone_number` = ?, `religion` = ?, `nationality` = ? WHERE `id` = ?",
            (str(f_name.get()), str(m_name.get()),  str(l_name.get()), str(gender.get()), int(age.get()), str(home_address.get()),
            str(phone_number.get()), str(Email.get()), str(nationality.get()), int(id)))
        conn.commit()
        cursor.execute("SELECT * FROM `contactable` ORDER BY `last_name` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def UpdateContactWindow(event):
    ...
    lbl_Religion = Label(ContactForm, text="Email", font=('arial', 14), bd=5)
    ...
    Religion = Entry(ContactForm, textvariable=Email, font=('arial', 14, 'bold'), bd=10, width=20, justify='left')
...
tree = ttk.Treeview(TableMargin, columns=("Id", "First Name", "Middle Name", "Last Name", "Gender", "Age", "Home Address", "Phone Number", "Email", "Nationality"),
